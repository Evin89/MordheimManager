...
## 26. Growth follow-ups — admin accuracy, onboarding, acquisition ⚠️ (built; migration 0048 pending, battle-filing bug open)
...
- ✅ **§26.6 report:** (1) yes — the warband screen's *Start battle* works without a campaign, and results go to a personal battle log (`campaign_id` null) listed on the Campaign tab; (2) only pre-battle opponent picking and scenario progress gating read campaign data. **Bug found (not fixed):** `useLogBattleMutation` files every battle under the user's *active* campaign, not the warband's own — a campaign-less warband owned by someone who is in a campaign has its battles logged into that campaign.
