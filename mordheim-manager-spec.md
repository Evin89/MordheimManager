...
## 26. Growth follow-ups — admin accuracy, onboarding, acquisition ✅
...
- ✅ **Supabase dashboard — checked and fixed 2026-09-25** (via `supabase config pull` into a scratch folder, plus a probe of `/auth/v1/verify` with a deliberately invalid token, which redirects to whatever URL the server resolves):
  1. **Site URL is `mordheimmanager.net` — no scheme. This is the confirmation bug.** A link without an explicit `redirect_to` falls back to the Site URL, which the server treats as a relative path and sends to `https://gkrekytyfvqyqubrsnov.supabase.co/mordheimmanager.net` — a Supabase 404. Sign-up sent no `emailRedirectTo` until §26.11, so **every confirmation email before that landed on a dead page**. The token was still consumed and the account confirmed; the user just saw an error and was never signed in. The landing-page forwarder can't help, since those links never reach our domain. ✅ **Fixed:** Site URL set to `https://mordheimmanager.net`; re-probed — a link with no or a non-allow-listed `redirect_to` now lands on `https://mordheimmanager.net/` (the landing page forwards it into the app).
