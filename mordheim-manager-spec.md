...
- ✅ **Chunk-load recovery.** Every route screen in `App.tsx` is `React.lazy`-loaded. The fix above keeps the HTML shell itself fresh, but not an open tab that was already sitting on the *previous* shell when a deploy replaced the build's hashed chunk filenames: an in-app link (client-side navigation, no HTML fetch) that then tries to `import()` a route chunk which no longer exists 404s, and with no error boundary above `<Suspense>` that unmounted the whole tree to a blank page — reported live as "the app goes black on a link click, but reloading fixes it." `ChunkErrorBoundary` (`src/components/ChunkErrorBoundary.tsx`) wraps the router's top-level `<Suspense>` and catches this specific failure (matched by message, since browsers throw it as a plain `Error` with no shared class): it reloads once automatically, silently picking up the current shell and chunk manifest, and only falls back to a visible "Reload" prompt if a second consecutive load still fails — which means a genuinely broken deploy, not a stale tab. Verified by throwing the real error text from a test route and watching it auto-recover on the first hit and land on the fallback on the second, deliberately-unrecovered one.
...
- ✅ **Warband rating** (`src/lib/rating.ts`) = (models × 5) + accumulated XP; large creatures count **20**. Dead, captured and departed models are excluded.
  ...
  - ✅ **Hired Swords use the rulebook's own flat per-type bonus**, not the generic formula. `hiredSwords.json` already carried the real figure as free text ("+22 points, plus 1 point per Experience point he has." for a Pit Fighter) — the gap was never sourcing, only that nothing read it: a structured `ratingFlatBonus` field was added alongside the display text (one per entry, extracted from it — the format is identical across all seven), and `computeWarbandRating` now looks it up by `sword.type`, which `createHiredSwordFromDefinition` already sets to the definition's own `name`, so the two can't drift apart. Falls back to the generic 5/20 formula for a type that doesn't resolve (a future entry added without a confirmed figure), rather than guessing. Verified live: hiring a Pit Fighter now raises rating by 22, not 5.
...
### 4.3 (revised) — Battle flow, playtest revisions ✅ (built)
...
**As built** — shipped to the three proposed defaults (tally-only enemy casualties, prompt-on-drop, `localStorage`-only), with two scoped deviations from the sketch below, both called out inline:

- **One persistence key, not per-warband.** The Zustand battle-session slice persists under a single `mordheim.battleDraft` key holding every warband's draft (each entry restores per warband exactly as a per-warband key would), rather than `mordheim.battleDraft.{warbandId}`.
- **Wizard step-progress is not persisted.** Only the during-battle session persists; the post-battle wizard re-derives its draft from that restored session on entry (feeding enemy-OOA → XP and wyrdstone → income). So a refresh **mid-wizard** restarts the wizard from the restored session rather than resuming on the same step — the reported bug (losing during-battle state) is fixed; mid-wizard resume was not in scope.

The two `TODO: verify vs rulebook` items (the exact per-enemy XP wording, and which scenarios carry counters) remain open — per §3.3 they are not set from memory. Alongside this, the during-battle and shared-roster cards gained skills and special-rules disclosures (matching the equipment rows) — a separate readability change, not part of this revision.

#### 4.3.1 During-battle screen ✅ (expanded)
...
##### A. Enemy taken out of action ✅ (built)
...
✅ **Resolved — tally only.** Built as the lightweight per-hero counter; *which* enemy models fell is not recorded in the battle record. Revisit only if both players routinely use the app. The stepper is placed on heroes **and hired swords** (both single models that earn XP), not henchmen groups.

##### B. Wyrdstone counters ✅ (built)
...
**What's built:** a **per-model wyrdstone-shard counter** on the during-battle screen, behind a toggle. The §3.3 flag is unverified and unset, so as built the counter is **off by default and revealed on demand** (and would auto-show once a scenario is flagged). Increment when a model picks a counter up, decrement when it's handed off or dropped.
...
- **On out-of-action:** when a model carrying shards is marked OOA, the app flags the carried count as **dropped** rather than silently zeroing or silently keeping it — the physical counter is on the table and its fate is a table decision. The player then reassigns it to whoever picked it up, or marks it lost.
  ✅ **Resolved — (a).** A downed single carrier's shards move to a **dropped pile** shown in the capture summary; the player reassigns by tapping **+** on whoever picked them up (which draws from the pile first), or **marks the pile lost**. Henchmen groups are left to the player, since "which member carried it" is unknown.
...
#### 4.3.2 Battle draft persistence ✅ (built) — the refresh fix
...
- A single **battle draft** (the during-battle session) captures the in-progress battle: pre-battle selections, during-battle OOA marks, enemy-OOA tallies, wyrdstone counters, and free-text notes. _(As built: the post-battle wizard's own step progress is **not** persisted — the wizard re-derives from this session on entry.)_
- It is persisted to **`localStorage`** under a single `mordheim.battleDraft` key holding every warband's draft, written on each change (not debounced — the writes are small and tap-frequency). Concretely: the battle-session slice of the Zustand store gets persistence via `zustand/persist`; the rest of the store stays transient.
...
✅ **Resolved — `localStorage` only.** A battle is played from one device in one sitting; a server-side draft (with its pre-commit write and RLS surface) was not built. A draft started on a phone does not appear on a tablet — acceptable for the reported bug.
...
**As built**, the persisted shape is the existing `BattleSession` (`src/store/useAppStore.ts`) extended with `startedAt`, `enemyOutOfAction`, `wyrdstoneCarried` and `droppedWyrdstone`, rather than a new `BattleDraft`. It keeps the session's existing fields (`scenario` as a name, `opponentWarbandId`, `opponentName`, `turn`, `events`, and `outOfAction` as an `OutOfActionTally`), and `wizardStep`/`wizardState` were **not** added — wizard progress isn't persisted (see the as-built note at the top of this section). The sketch above is the design intent.
...
#### Resolved decisions ✅ (built to the proposed defaults)

1. **Enemy casualties in the log** (§4.3.1 A) — ✅ tally only; which enemy models fell is not recorded.
2. **Wyrdstone drop-on-OOA** (§4.3.1 B) — ✅ prompt: a downed carrier's shards go to a dropped pile, reassigned on pick-up or marked lost.
3. **Cross-device resume** (§4.3.2) — ✅ `localStorage` only.

Still open, pending rulebook verification (not set from memory, per §3.3): the exact per-enemy XP wording (§4.3.1 A), and which scenarios carry wyrdstone counters (§4.3.1 B / §3.3 `usesWyrdstoneCounters`).
...
- ✅ **Data-file version display** and **"report a data error" link** — a *Game data* card on the Account screen lists all 15 bundled data files (`name · v{schemaVersion}`), each a disclosure expanding to its rulebook `source`, plus a report form that files into the same `issue_reports` inbox as the rest of the app with the data versions attached as context (so an admin sees exactly what the reporter was running). Built from a small `src/lib/dataFiles.ts` registry using tree-shaken named JSON imports, so the large data blobs don't ride into the Account route. ◻️ The **strict-validation toggle** is deferred — there is no data-file validator to gate yet, and its semantics (what "strict" checks, and what a failure does) need defining before it's more than a switch that does nothing.
...
✅ **The two audit-log tables now have a ceiling** (migration 0043). `warband_edits` and `warband_rating_history` (§4.9.4, §18.3) are not actually unbounded by warband deletion — both cascade away when their parent warband is hard-purged (§10.5's job) — the real gap was a single warband kept alive and edited for years, accumulating rows with no cap at all. `purge_old_audit_logs()` deletes rows older than a parameterised window, defaulting to **3 years** — deliberately generous rather than aggressive, since `warband_rating_history` backs a real, visible feature (§18.3's rating chart) and a short window would quietly truncate it for any campaign that outlives the window. This exists to put a ceiling on literally-infinite growth, not to actively prune history anyone is still looking at; no Mordheim campaign here has run anywhere near three years. Same pg_cron pattern as §10.5's job (defensive extension check, an `admin_*` wrapper for a future manual trigger, not yet wired to a Maintenance-screen button).

✅ **RLS policies no longer re-evaluate `auth.uid()` per row** (migration 0042). Supabase's own advisor flagged 45 policies project-wide, discovered in the same audit, where a bare `auth.uid()` inside a `USING`/`WITH CHECK` clause is re-derived once per row scanned rather than once per query — invisible at this app's row counts, real at scale. 44 of the 45 (the 45th, `warbands_delete_own`, was already gone — dropped outright by §10.4's own migration 0041) are rewritten to `(select auth.uid())`, reproduced verbatim from each policy's authoritative, most-recently-redefined `create policy` statement with only the wrapping changed — no USING/WITH CHECK logic altered, so this changes performance characteristics only, never who can read or write what.
...
- ✅ **Optimistic updates** for small edits (XP +1, gold change): `useSaveWarbandMutation` writes the edit into the cache on `onMutate` (cancelling in-flight refetches and snapshotting for rollback), reconciles the server's fresh `updated_at` on success **without** invalidating (so no refetch per tap), and rolls back to the snapshot on error. Covers every discrete warband edit that routes through the save — XP, gold, buys, sells, advances, injuries. The optimistic-concurrency sequencing (§8.4) is unchanged: the record's `updated_at` is still read inside the queued save task.
...
- **Bloodiest** (most models lost) ✅ **and Slayer** (most enemies taken out) ✅ — both now built. They read structured counts the post-battle wizard records — `modelsLost` (derived from the battle's own deaths) and `enemiesTakenOut` (self-reported) — added to `BattleRecord` on the jsonb blob, so no migration was needed and older records read as zero.
...
✅ **Resolved** — rather than parse the free-text `casualtiesSummary`, the wizard records structured `modelsLost` and `enemiesTakenOut` counts on `BattleRecord` (jsonb, no migration; older rows read as zero). Bloodiest and Slayer compute from those. The during-battle enemy-OOA tally (§4.3.1 A) feeds `enemiesTakenOut`; deaths at commit feed `modelsLost`.
...
✅ **Built** (migration 0016). The trigger fires `after insert or update of rating`, guarded by `is distinct from` so a save touching only gold or a name plants no duplicate point. One backfill row per existing warband at its current rating, so a warband with history doesn't start its chart at a single dot today — the honest amount, since the path that led there was never recorded and isn't invented (cf. 18.2). The chart is hand-drawn SVG on the warband detail screen, collapsed under the rating figure and shown only past two points (one is a dot, not a trend) — the same no-dependency choice as the admin signups sparkline.

✅ **The standings-tab comparison view is now built too** — `CampaignRatingChart`, one overlaid line per entered warband, shown on the Standings tab whenever at least two warbands are entered (a lone one has nothing to compare against, and its own detail screen already carries the single-line version, left completely unchanged per the product call to keep that one simple). Two decisions worth recording:

- **Time-based x-axis, not point-index.** Two warbands' ratings change on different battle nights, so the detail chart's `i / (points.length - 1)` index spacing would silently misalign the calendar if reused here — warband A's 3rd point and warband B's 3rd point are not the same date. The comparison chart scales every point by its real `recorded_at` timestamp against one shared time domain instead.
- **A genuine categorical palette, added to the token system rather than hardcoded.** This is the app's first multi-series chart, and the existing single-accent chart style (one ember line) had nothing to reuse. Six new theme-aware tokens (`chart-1`…`chart-6`, `index.css` + `tailwind.config.js`, following the exact pattern `--color-accent`/`--color-verdigris` already set) — muted, "aged ink" hues (gold, slate blue, dusty plum, olive-sage, terracotta, grey-teal) rather than a bright modern chart palette, chosen to avoid any hue already carrying meaning elsewhere (nothing orange/red near ember or blood, nothing blue-green near verdigris) and to fit the Rulebook aesthetic's general avoidance of saturated colour. Colour is assigned by warband *identity* (alphabetical order), never by current rank — a warband's line can't change colour just because someone overtook it. A campaign beyond six entered warbands folds the rest into the existing `ink-faded` token rather than a seventh invented hue, so a large campaign's chart fades out gracefully instead of running out of distinguishable colour; the legend still names every warband regardless. ✅ **Measured, not eyeballed.** `design-sheet.mjs` now checks the chart palette too — against WCAG 1.4.11's 3:1 non-text floor (a chart line is a graphical object, not body text, so 1.4.3's 4.5:1 is the wrong bar) rather than the page background these lines actually sit on (`bg-ink-950` in `index.html`, which the parchment theme redefines to the parchment value — the same background in both themes by construction). All 12 (six hues × two themes) clear it: weakest is chart-1 on Rulebook's parchment at 4.04:1, strongest is chart-1 on Grimdark's near-black at 8.52:1. `npm run design-sheet` fails the build if a future edit drops one below the floor, the same guarantee the rest of the palette already had.
- ⚠️ **The app's first interactive chart.** Shipped static at first — no hover, matching every other chart in the app — then given a hover crosshair + tooltip on request, still no chart library. Built on **Pointer Events, not `mouse*`**, because the app is mobile-first and a mouse-only hover would simply never fire at the table: a mouse tracks continuously (`pointerType === 'mouse'`), a touch only tracks while actively pressed (`pointerdown` + drag, matching common mobile chart UX — tap and drag to scrub, lift to dismiss) rather than eating every scroll-past touch on the tab. The tooltip shows each warband's rating **as of the hovered date via a step lookup (the last point at-or-before that time), never interpolated** between two points — a rating changes on a battle night, not continuously, so guessing at an in-between value would state something the data never claimed. Flips from the chart's left to right edge past the horizontal midpoint so it can't run off either side. The always-visible legend stays, both as the non-hovering state's summary and because colour is never the only way to tell two lines apart.
...
| 7 | ✅ §18.3 Rating history | Built (migration 0016; detail-screen chart, plus the standings-tab comparison chart) |
...
| `playstyleSummary` / `archetype` | **Derived, tier 2 of the brief's own fallback chain** (§5.4.5: "derive a structural blurb … else hide"). `deriveWarbandPlaystyle()` recombines only data already on the definition — hero/henchmen type counts, the first one or two named special rules quoted verbatim, whether a Hero casts, whether skill access runs uniformly wide or uniformly narrow — into one or two sentences, e.g. *"3 Hero types and 3 Henchmen types, shaped by Military discipline and Marksmanship."* Returns null (hidden, not a generic filler line) when a warband has neither heroes nor henchmen types to describe. No mechanic or number is stated that the data doesn't already carry. Tier 1 (an authored field) still doesn't exist — trivial to switch on later without schema churn, per the brief's own §5.4.15 item 7, since this tier only runs when tier 1 is absent |
...
The brief's own §5.4.15 listed five resolved decisions and four open ones (route/section-number, `playstyleSummary`/`archetype`, the special-rules boxing trigger, and the relationship to warband-compare). All five resolved items were built as specified. Of the four open ones: route was decided for this app's actual constraints (§25.1), `playstyleSummary`/`archetype` now runs the brief's own tier-2 derive fallback (§25.2), the boxing trigger uses the stated default allowlist (§25.3), and the relationship to §20.2's warband-compare tool is **left separate**, matching the brief's own stated default — no CTA between the two was added.

### 25.6 Status ✅

Built and wired: the route, the New Warband screen's `?type=` preselect for the CTA, and the Warbands tab's Rules panel (a searchable "Choose a warband…" picker, replacing a plain `<select>`, since a picker at 49 warbands needs to be searchable the same way §4.1's own type picker already is).

✅ **The two pages are consolidated.** The older per-warband rules page — an inline render inside `RuleDetailScreen`, reachable at `/rules/warband-:id` — is retired. Its ids are still generated by §4.8's search index (so search and the chapter accordion still find every warband by name), but `RuleDetailScreen` now redirects a `warband-*` id straight to this screen (`/rules/warbands/:id`) instead of rendering a competing, thinner view. The old view's dedicated `WarbandRosterDetail` component, left with nothing left to render, was removed rather than kept dormant.

## 26. Growth follow-ups — admin accuracy, onboarding, acquisition ✅ (Supabase dashboard checks remain — §26.12)

_Drafted 2026-09-25 from a read of the live admin screens (Overview + Players) at 47 players / 85 warbands / 45 battles / 6 campaigns. Amends §4.9.2, §4.9.4 and §23.4 where noted (each carries a pointer back here); everything else is new._

**For Claude Code — read first.**
- Items marked **report-only** are audits: produce findings with file/line references, make no edits in the same pass.
- ❓ marks a genuine open decision or a fact this spec could not verify from the screenshots. Each carries a reasoned default; use the default unless Evin has overridden it.
...
```sql
greatest(
  p.last_seen_at,                                   -- the tracked value, if any
  (select max(w.updated_at) from warbands w
     where w.owner_id = p.id),                      -- includes soft-deleted: an edit is still a visit
  (select max(b.created_at) from battles b
     where b.reported_by = p.id),
  p.created_at                                      -- signing up is being seen
) as last_seen
```
...
**Fix.**
- The first column (player name + "joined …" line + ADMIN badge) is `position: sticky; left: 0` inside the table's horizontal scroll container, with an opaque background from the existing surface token (never transparent, or the scrolled cells show through) and a `z-index` above body cells.
- The header row is `position: sticky; top: 0` within the same container. The top-left header cell is sticky on both axes and sits above both.
- Separate the sticky column with a 1px right border using the existing rule/border token. **No gradient or shadow fade** — the same class of decoration was tried and removed in Mord Hive for mobile fragility (see ways-of-working).
...
**26.3.1 Admin diagnostic fields.** Extend `admin_user_overview()` (list + detail) with metadata read from `auth.users`:

| Field | Source | Why |
| --- | --- | --- |
| `email_confirmed` | `auth.users.email_confirmed_at is not null` (boolean only) | An unconfirmed account can't sign in; this separates "blocked" from "left". |
| `last_sign_in_at` | `auth.users.last_sign_in_at` | Distinguishes "signed in, did nothing" from "never got in". |

These are booleans/timestamps, not email addresses, so §4.9.7 holds. The email itself is never selected.

Surface them on the player detail screen, and as an **Overview attention badge**: "Unconfirmed accounts: N" (same pattern as open issues / purge queue in §4.9.1).

**26.3.2 Auth flow audit — report-only.** The app moved domains (subdomain → `mordheimmanager.net`), which is exactly when auth redirects break. Check and report:

1. Supabase **Site URL** and **Redirect URLs allow-list**: do confirmation and reset links point to the current production domain? Any leftover old-domain or preview entries?
2. **Confirmation email**: does the link land on a route that completes the session and shows success, or on a blank/404/generic page?
3. **Sign-in while unconfirmed**: what error does the user see? Is there a "resend confirmation email" action?
4. **Password reset**: does a reset route exist end-to-end (request → email → set new password → signed in)?
5. **Sign-up with an already-registered email**: what does the user see? (Supabase can return a success-shaped response here by design; the UI must still point them to sign-in/reset.)
6. **Rate limits / SMTP**: is the project on Supabase's built-in email sender (low hourly limit, easily hit during a signup spike) or custom SMTP?

After Evin reviews the report, fixes follow as a separate pass.
...
**Claude Code — report:**
1. Can a battle be logged for a warband that isn't in a campaign today? If so, where is the entry point, and is it reachable from the warband screen?
2. If battles require a campaign: which parts of the battle flow (§4.3, including the playtest revisions) actually depend on campaign data, and which would work for a lone warband?
...
**26.7.2 Self-reported source.** Referrers miss most Discord traffic, so ask as well. On the register form, add one **optional** field below the main inputs:

> How did you find Mordheim Manager? _(optional)_
> Discord · Reddit · mordheimer.net · My gaming group / a friend · Search engine · Other
...
- **Talk to the active campaign group.** They're the only people using the battle flow. Ask what works, what's annoying, and what they'd skip. The answer decides §26.6.
- **Tag every link you post.** Use `?ref=discord`, `?ref=reddit`, `?ref=mordheimer` (§23.8). This works from today, with no code change.
- **Search for mentions** of the app in r/Mordheim and the larger Mordheim Discords to find the source of the current spike.
- **Follow up on `G4bri3l`** by hand if there's a channel for it, once §26.3.1 shows whether that account is unconfirmed.

### 26.9 Build order

| # | Item | Size | Why this position |
| --- | --- | --- | --- |
| 1 | §26.1 last seen + shared `user_last_seen()` | S | Fixes the numbers every later item is judged by |
| 2 | §26.2 sticky column | S | Makes the Players screen usable on the device Evin checks it on |
| 3 | §26.3.2 auth audit (report) → §26.3.1 fields | S + fixes | A broken confirm/reset flow silently costs signups |
| 4 | §26.7.1 acquisition verify (report) → §26.7.2 self-report | S–M | Answers "where do they come from" |
| 5 | §26.4 onboarding | M | Biggest funnel lever once 3 has ruled out auth failures |
...
- ✅ **§26.1** — cause confirmed: `last_seen_at` arrived in 0037 (2026-09-15) and is written only on app load, going forward. `user_last_seen(uid)` (0045) derives last seen at read time; `admin_user_overview()`, `admin_stats()`'s *Online today* / *Active · 7d* and `admin_retention_cohorts()` all call it. Not granted to `anon`/`authenticated` (it would leak presence for any uuid). Side effect on retention: signup is a floor, so week 0 now reads 100%.
- ✅ **§26.2** — sticky player column + sticky header in one scroll container (`max-h-[75vh]`), opaque `ink-900`, 1px rule, no fade; the table is `border-separate` because collapsed borders don't travel with a sticky cell. Name column capped at 40vw.
- ✅ **§26.3.1** — `email_confirmed` + `last_sign_in_at` on the Players list (an *unconfirmed* chip) and player detail; `unconfirmed_users` on `admin_stats()` feeds an Overview attention badge (0046). The address is never selected.
- ✅ **§26.3.2 audit (report):**
  1. *Site URL / redirect allow-list* — not verifiable from the repo (no `config.toml`; dashboard-only). The code sends Google to `/app`, resets to `/app/reset-password`, and passes **no `emailRedirectTo` on signup**, so confirmation links go to whatever the Site URL is.
  2. *Confirmation link* — lands on the Site URL. If that's the root, it's the static `landing.html`, which has no supabase-js: the account *is* confirmed server-side, but the user isn't signed in and sees no success message.
  3. *Sign-in while unconfirmed* — shows Supabase's raw "Email not confirmed"; there is **no resend-confirmation action** anywhere.
  4. *Password reset* — works end to end (request → email → `/reset-password` → signed in).
  5. *Sign-up, and sign-up with a registered email* — **the most likely cause of zero-warband accounts:** after *any* successful `signUp` the register screen navigates to Home, and with confirmation on there's no session, so the user lands on the signed-out Home with **no "check your email" message**. A duplicate email gets Supabase's success-shaped response and the identical silent landing.
  6. *SMTP / rate limits* — not verifiable from the repo.
  ✅ **Fixed after review (§26.11).**
- ✅ **§26.4** — no `track.ts` or `app_events` exists (the spec's premise was wrong); the two onboarding events joined the consent-gated PostHog union instead, and `warband_created` gained `is_first`. The success measure — the funnel's stage 1→2 — is DB-derived regardless. First-run landing: a `sessionStorage` flag set at password/Google/sessioned sign-up, consumed once by Home (redirects to `/warbands/new` when the user has zero warbands; a sign-in returning to a specific page clears it). Joining a campaign is code-entry only (no join-by-link path), so the invite exception has nothing to hook; the empty-state Home links to `/campaigns` for a join code instead. Empty-state Home: the Get Started card leads, then rules/join-code/Discord. The existing §20.4 nav tour no longer auto-offers before the first warband.
- ✅ **§26.5** — `ShareNudgeCard` on the owner's warband screen: private + has a logged battle (campaign or personal log) + not dismissed; "Make public" hands over a `?ref=share-link` URL. **Found and fixed on the way:** the visibility switch lived inside `WarbandSharingCard`'s "in a campaign" branch, so an owner outside any campaign had no way to make a warband public at all. Also corrected the public option's copy (it's anon-readable, not "anyone signed in").
- ✅ **§26.6 report:** (1) yes — the warband screen's *Start battle* works without a campaign, and results go to a personal battle log (`campaign_id` null) listed on the Campaign tab; (2) only pre-battle opponent picking and scenario progress gating read campaign data. **Bug found, fixed in §26.12:** `useLogBattleMutation` filed every battle under the user's *active* campaign, not the warband's own — a campaign-less warband owned by someone who is in a campaign has its battles logged into that campaign.
- ✅ **§26.7.1 report** (leaks fixed in §26.11): (1) the §23.5 panel set is fully shipped — the single-series signup bars are the older Overview sparkline; the multi-metric Activity series and Acquisition panel sit below it in *Growth*. (2)/(4) live counts not run — no database access from this session. (3) **Capture has three leaks:** the stash is `sessionStorage`, not `localStorage` (lost when the tab closes); the static landing page doesn't capture at all, so `mordheimmanager.net/?ref=discord` → *Create an account* drops the tag and then the landing page itself becomes the referrer, classified **`other`**; and Google sign-ups carry no acquisition metadata (0032, accepted at the time).
- ✅ **§26.7.2** — optional self-report on the register form (closed set + ≤80-char "Other" note), written once by `handle_new_user` (0047). Read by a new `admin_acquisition_self_report()` rather than reshaping `admin_acquisition_breakdown()`, whose bare-array result the live screen reads. **Also closed two §23.4 holes:** 0038's column grant had re-exposed every `acquisition_*` column to `anon`/`authenticated`, and the default table-wide UPDATE grant let users rewrite their own acquisition/presence columns — now SELECT on public columns only, UPDATE on `display_name` only. Google sign-ups skip the form and so never self-report.

### 26.11 Auth and acquisition fixes ✅

The §26.3.2 and §26.7.1 findings, fixed after review.

**Auth.**
- **Sign-up says what happens next.** A signup that returns no session (confirmation on — and, indistinguishably by design, an address that already has an account) now shows *Check your email* with the address, a spam hint, a **resend** button, and a neutral line: already registered? then no email is coming — sign in or reset. The old path dropped everyone on the signed-out Home in silence.
- **Confirmation links come back into the app, signed in.** `signUp` and `resend` pass `emailRedirectTo: <origin>/app` (the URL the Google flow already has on the allow-list), so supabase-js completes the session from the hash. The arrival (`type=signup` in the hash, read before the client wipes it) counts as a first sign-in, so §26.4.1's first-run landing applies.
- **Links that land on the landing page are forwarded.** Emails already sent point at the bare Site URL — the static landing page, which ignored the token. An inline script at the top of `landing.html` forwards any `#access_token` / `#error…` / `?code=` to `/app` intact.
- **Failed links explain themselves.** An expired or already-used link lands on signed-out Home with a one-line notice pointing to sign-in and resend.
- **Sign-in while unconfirmed** shows a plain explanation and a resend button instead of Supabase's raw "Email not confirmed".
- Still dashboard-only, unverifiable from the repo: the Site URL / allow-list entries and the email sender. With custom SMTP absent, Supabase's built-in sender's low hourly cap also limits resends.

**Acquisition.**
- The first-touch stash is **localStorage with a 30-day expiry** (was sessionStorage — lost when the tab closed), and cleared once an account records it, so a second account on the device doesn't inherit it.
- **The landing page captures** a tagged or externally-referred visit into a raw key the app classifies on first load.
- **Our own hosts never count as a referrer**, so arriving through the landing page no longer files a signup under `other`. An untagged arrival with an external referrer is stashed too.
- **Google sign-ups record their source** via `record_signup_acquisition()` (0048): own row only, write-once, only within 30 minutes of the profile's creation, closed channel set, capped strings, returns nothing — a fenced one-shot, not a general client write path. Google sign-ups still don't self-report (they skip the register form).

### 26.12 Remaining follow-ups ✅ (dashboard checks for Evin)

- ✅ **Battles file under the warband's own campaign.** `useLogBattleMutation` now looks up the logged warband's `campaignId` and inserts under that — or as a personal battle (`campaign_id` null) when the warband isn't entered anywhere, or its campaign is one the player no longer belongs to (the insert policy requires membership, and a stale link mustn't lose the battle). Battles already misfiled under the old rule are left where they are: a warband that was later *removed* from a campaign looks the same in the data, so there's no safe automatic correction.
- ✅ **Google sign-ups get the source question.** A one-time inline card on Home and the New Warband screen (where first-run lands) asks a Google account under a day old the same question as the register form (shared `SelfReportFields`). Save or *No thanks* retires it on the device. Written by `record_signup_self_report()` (0049): own row, write-once, within 24 hours of creation, closed set, note only for "Other", capped at 80 — returns nothing.
- ⚠️ **Supabase dashboard — checked 2026-09-25** (via `supabase config pull` into a scratch folder, plus a probe of `/auth/v1/verify` with a deliberately invalid token, which redirects to whatever URL the server resolves):
  1. **Site URL is `mordheimmanager.net` — no scheme. This is the confirmation bug.** A link without an explicit `redirect_to` falls back to the Site URL, which the server treats as a relative path and sends to `https://gkrekytyfvqyqubrsnov.supabase.co/mordheimmanager.net` — a Supabase 404. Sign-up sent no `emailRedirectTo` until §26.11, so **every confirmation email before that landed on a dead page**. The token was still consumed and the account confirmed; the user just saw an error and was never signed in. The landing-page forwarder can't help, since those links never reach our domain. **Fix (dashboard):** Authentication → URL Configuration → Site URL `https://mordheimmanager.net`.
  2. **Redirect URLs:** the explicit list came back empty, but `https://mordheimmanager.net/app` and `…/app/reset-password` are accepted (probe-verified), so §26.11's confirmation, resend, reset and Google redirects all work today. Once the Site URL is fixed, add both to the allow-list explicitly anyway, rather than relying on host matching against a malformed Site URL.
  ...
  4. Templates weren't comparable through the CLI; check the Confirm signup / Reset password templates still match `public/mailTemplates/`.
