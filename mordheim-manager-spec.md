...
- ⚠️ **Supabase dashboard — Evin to check (not readable from the repo or this session):**
  1. *Authentication → URL Configuration:* Site URL `https://mordheimmanager.net` (the landing page now forwards auth links either way); Redirect URLs include `https://mordheimmanager.net/app` (confirmation, resend, Google) and `https://mordheimmanager.net/app/reset-password`. Remove leftover `mordheim.builderbasement.com` and preview entries once that host is retired.
  2. *Authentication → Emails → SMTP Settings:* if custom SMTP is off, Supabase's built-in sender allows only a handful of auth emails per hour across the whole project — a signup spike or a few resends exhausts it and confirmation emails silently stop. Set up a custom SMTP provider (Resend, Postmark, SES, Brevo…) with a sender on `mordheimmanager.net`, then raise *Rate Limits → emails sent per hour*.
  3. *Authentication → Emails → Templates:* the Confirm signup and Reset password templates should be the ones in `public/mailTemplates/` (they use `{{ .ConfirmationURL }}`, which honours the redirect above).
