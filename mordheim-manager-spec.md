...
- **Deliberately static — no hover, no tooltip, no chart library.** A generic multi-series line chart convention calls for a hover crosshair and per-point tooltip; this app's charts have never had one (the detail-screen line, the admin signups sparkline), and introducing the first interactive chart for this one view was weighed and declined in favour of consistency with the rest of the product. A legend below the chart (swatch, name, latest rating) carries the same information a tooltip would, just always-visible instead of on-hover.
