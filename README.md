...
- ✅ **Rules Reference** — a searchable, offline rules browser (its own bottom-nav tab) covering Characteristics, the Turn sequence, Movement, Shooting, Close Combat, Wounds & Injuries, Leadership & Psychology, Magic/casting (plus the wizard-to-lore allocation table), the Post-Battle Sequence, Trading & Hired Swords, and — generated live from the app's own data rather than duplicated — Skills, Injuries, Scenarios, Warband-Specific Rules, and Border Town Burning objectives/Dramatis Personae. Entries cross-link to related rules, and known gaps (the Strength-vs-Toughness wound chart, per-lore spell lists, full scenario terrain/deployment text) are flagged explicitly instead of guessed.
