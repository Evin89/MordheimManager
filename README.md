...
- **Public warband browsing.** Warbands can be marked `public` and RLS already lets any signed-in user read them, but there's no screen that lists them — the setting currently has no reachable consumer.
- **One data gap found while auditing purchases.** The Maneaters' hand-held mortar has no price in `equipment.json`, so its buy field prefills empty. Everything else parses: of 108 purchasable items only Gromril and Ithilmar weapons resolve to no fixed price, correctly, since they cost a multiple of the base weapon.
